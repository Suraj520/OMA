...
public class Matrix {
    ...
    public static void multiplyMM(float[] result, int resultOffset,
        ...
        if(result == null || lhs == null || rhs == null){
            throw new IllegalArgumentException("Errore null");
        }

        if(resultOffset+16 > result.length){
            throw new IllegalArgumentException("Errore result offset");
        }

        if(lhsOffset+16 > lhs.length){
            throw new IllegalArgumentException("Errore lhs offset");
        }

        if(rhsOffset+16 > rhs.length){
            throw new IllegalArgumentException("Errore rhs offset");
        }

        for (int i=0 ; i<4 ; i++) {
            float rhs_i0 = rhs[ 4*i ];
            float ri0 = lhs[ 0 ] * rhs_i0;
            float ri1 = lhs[ 1 ] * rhs_i0;
            float ri2 = lhs[ 2 ] * rhs_i0;
            float ri3 = lhs[ 3 ] * rhs_i0;

            for (int j=1 ; j<4 ; j++) {
                float rhs_ij = rhs[ j+4*i ];
                ri0 += lhs[ 4*j ] * rhs_ij;
    ...
    public static void multiplyMV(float[] resultVec,
        ...
        if(resultVecOffset+4 > resultVec.length){
            throw new IllegalArgumentException("Errore resultVec offset");
        }

        if(lhsMatOffset+16 > lhsMat.length){
            throw new IllegalArgumentException("Errore lhsMat offset");
        }

        if(rhsVecOffset+4 > rhsVec.length){
            throw new IllegalArgumentException("Errore rhsVec offset");
        }

        mx4transform(rhsVec[0], rhsVec[1], rhsVec[2], rhsVec[3], lhsMat, resultVec);
    }

    private static void mx4transform(float x, float y, float z, float w, float[] pM, float[] pDest) {
